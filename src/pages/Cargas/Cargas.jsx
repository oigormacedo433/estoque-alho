...
function montarMapaEstoque(estoqueAtual = []) {
  ...
  estoqueAtual.forEach((item) => {
    ...
    atual.saldo_caixas += numero(
      item.saldo_disponivel_caixas ||
        item.saldo_caixas ||
        item.disponivel_caixas ||
        item.estoque_caixas
    );
...
function analisarEstoqueCarga(carga, estoquePorCalibre) {
  ...
  const detalhes = itens.map((item) => {
    ...
    const disponivel = numero(estoque?.saldo_caixas);
...
export default function Cargas() {
  ...
  const estoquePorCalibre = useMemo(() => {
    return montarMapaEstoque(estoqueAtual);
  }, [estoqueAtual]);
              ...
              <section className="rounded-2xl border border-slate-100 bg-slate-50 p-5">
                ...
                <div className="space-y-4">
                  {formulario.itens.map((item, index) => {
                    const estoque = estoquePorCalibre.get(item.calibre_id);
                    const saldoDisponivel = numero(estoque?.saldo_caixas);
                    const quantidadePedida = numero(item.quantidade_caixas);
                    const possuiCalibre = Boolean(item.calibre_id);

                    let mensagemEstoque = "Selecione um calibre para ver o estoque atual.";
                    let classeMensagemEstoque = "border-blue-200 bg-blue-50 text-blue-700";

                    if (possuiCalibre && saldoDisponivel <= 0) {
                      mensagemEstoque = "Sem estoque atual para este calibre.";
                      classeMensagemEstoque = "border-red-200 bg-red-50 text-red-700";
                    }

                    if (possuiCalibre && saldoDisponivel > 0) {
                      mensagemEstoque = `Estoque atual: ${formatarNumero(saldoDisponivel)} caixas.`;
                    ...
                    if (
                      ...
                      mensagemEstoque = `Estoque parcial: ${formatarNumero(
                        saldoDisponivel
                      )} caixas disponiveis para ${formatarNumero(quantidadePedida)} solicitadas.`;
