...
function replaceFunctionByName(functionName, newFunctionCode) {
  ...
  if (functionStart >= 0) {
    ...
    if (paramsOpen < 0 || paramsClose < 0 || bodyOpen < 0 || bodyClose < 0) {
      throw new Error(`Nao consegui substituir a funcao ${functionName}.`);
    }
...
function findChartComponentName() {
  ...
  const chunk = content.slice(titleIndex, titleIndex + 5000);
...
function patchEstoqueCell() {
  ...
  if (!keyMatch) {
    throw new Error("Nao encontrei a linha da tabela de cargas.");
  }
  ...
  if (tdMatches.length < 7) {
    throw new Error("Nao encontrei a coluna Estoque.");
  }
...
console.log("OK: arquivo Cargas corrigido sem quebrar JSX.");
